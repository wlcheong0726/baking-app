...
- New branches created for frontend and backend to containerise each service.
- Containerised the application: created Dockerfiles for backend and frontend, then merged branches into develop.
- Orchestrated both services using Docker Compose for local multi-service development.

## 28 Nov 2025 - 29 Nov 2025

BACKEND:

- new branch: feature/recipe-external-api.
- Used feign client to call external api - Tasty, for dessert recipe.
- Built a layered architecture for recipes, mimicking Blogs, currently one method to retrieve dessert recipes.
- Tested with Postman - working.

FRONTEND:

- refactoring code to use Tailwind css for styling as opposed to pure css for efficiency.
- refactored BlogsPage.

## 01 Dec 2025

FRONTEND:

- new branch: refactor/use-tailwind-css
- Refactored components to use tailwindcss in all blog components
- Added some animation in index.css for use in BlogCard component

BACKEND:

- Updated data.sql with some emojis and content length with some more and less than 200 characters to test Read More button.
```
